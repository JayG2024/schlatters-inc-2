...
import { clientHours, teamMembers, mockTimeEntries, formatDuration, categoryHours } from './mockData';
...
const BillableHours: React.FC = () => {
  ...
  const unbilledHours = mockTimeEntries
    .filter(entry => entry.billable && entry.endTime && !entry.invoiced)
    .reduce((acc, entry) => acc + (entry.duration || 0), 0) / 60;
  ...
  return (
    <div className="space-y-6">
      ...
      <Card className="bg-white">
        ...
        <CardContent>
          <div className="space-y-6">
            {Object.entries(clientHours).map(([client, hours]) => {
